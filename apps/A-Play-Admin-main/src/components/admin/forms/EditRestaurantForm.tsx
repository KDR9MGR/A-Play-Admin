...
export function EditRestaurantForm({ restaurant, onClose, onSuccess }: EditRestaurantFormProps) {
  ...
  useEffect(() => {
    if (restaurant) {
      setFormData({
        ...
        price_range: restaurant.price_range ? String(restaurant.price_range) : "",
        amenities: restaurant.amenities || [],
        cuisine_types: restaurant.cuisine_types || [],
        has_delivery: restaurant.has_delivery || false,
        has_takeout: restaurant.has_takeout || false,
        has_reservations: restaurant.has_reservations || false,
        seating_capacity: (restaurant as any).seating_capacity ?? "",
        total_tables: (restaurant as any).total_tables ?? "",
      });
    }
  }, [restaurant]);
  ...
  const updateRestaurantMutation = useMutation({
    mutationFn: async (restaurantData: typeof formData) => {
      const { data, error } = await supabase
        .from("restaurants")
        .update({ ...restaurantData, address: restaurantData.location, price_range: restaurantData.price_range ? Number(restaurantData.price_range) : null })
        .eq("id", restaurant.id)
        .select()
        .single();
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleInputChange("seating_capacity", e.target.value ? Number(e.target.value) : "")}
                  placeholder="e.g. 80 seats"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="total_tables">Total Tables</Label>
                <Input
                  id="total_tables"
                  type="number"
                  min="0"
                  value={formData.total_tables}
                  onChange={(e) => handleInputChange("total_tables", e.target.value ? Number(e.target.value) : "")}
