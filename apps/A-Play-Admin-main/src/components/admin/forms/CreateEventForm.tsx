...
export function CreateEventForm({ onClose, onSuccess }: CreateEventFormProps) {
  ...
  const createEventMutation = useMutation({
    mutationFn: async (eventData: typeof formData) => {
      // Create the event — category is tracked via the event_categories junction table only
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.start_date || !formData.end_date || !formData.description || !formData.location) {
      toast.error("Please fill in all required fields (including End Date)");
      return;
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleInputChange("club_id", value === "none" ? "" : value)}
                >
                  <SelectTrigger>
