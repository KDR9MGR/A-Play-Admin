  ...
  useEffect(() => {
    if (pub) {
      setFormData({
        ...
        location: pub.location || pub.address || "",
        cover_image: pub.cover_image || "",
        logo_url: pub.logo_url || "",
        phone: pub.phone || "",
        email: pub.email || "",
        website: pub.website || "",
        price_range: pub.price_range ? String(pub.price_range) : "",
  ...
  const updatePubMutation = useMutation({
    mutationFn: async (pubData: typeof formData) => {
      const { data, error } = await supabase
        .from("pubs")
        .update({ ...pubData, address: pubData.location, price_range: pubData.price_range ? Number(pubData.price_range) : null })
        .eq("id", pub.id)
        .select()
        .single();
