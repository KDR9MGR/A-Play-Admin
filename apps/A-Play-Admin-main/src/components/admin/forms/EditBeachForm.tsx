  ...
  useEffect(() => {
    if (beach) {
      setFormData({
        ...
        location: beach.location || beach.address || "",
        cover_image: beach.cover_image || "",
        logo_url: beach.logo_url || "",
        phone: beach.phone || "",
        email: beach.email || "",
        website: beach.website || "",
        price_range: beach.price_range ? String(beach.price_range) : "",
  ...
  const updateBeachMutation = useMutation({
    mutationFn: async (beachData: typeof formData) => {
      const { data, error } = await supabase
        .from("beaches")
        .update({ ...beachData, address: beachData.location, price_range: beachData.price_range ? Number(beachData.price_range) : null })
        .eq("id", beach.id)
        .select()
        .single();
