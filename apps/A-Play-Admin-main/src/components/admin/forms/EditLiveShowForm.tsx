...
interface EditLiveShowFormProps {
  liveShow: any;
  onClose: () => void;
  onSuccess: () => void;
}

export function EditLiveShowForm({ liveShow, onClose, onSuccess }: EditLiveShowFormProps) {
  ...
  useEffect(() => {
    if (liveShow) {
      setFormData({
        title: liveShow.title || liveShow.name || "",
        description: liveShow.description || "",
        performer_name: liveShow.performer_name || "",
        venue_name: liveShow.venue_name || "",
        location: liveShow.location || liveShow.address || "",
        show_date: liveShow.show_date ? new Date(liveShow.show_date).toISOString().slice(0, 16) : "",
        show_time: liveShow.show_time || "",
        duration_minutes: liveShow.duration_minutes ? String(liveShow.duration_minutes) : "",
        cover_image: liveShow.cover_image || liveShow.cover_image_url || "",
        genre: liveShow.genre || [],
        ticket_price_min: liveShow.ticket_price_min ? String(liveShow.ticket_price_min) : liveShow.ticket_price ? String(liveShow.ticket_price) : "",
        ticket_price_max: liveShow.ticket_price_max ? String(liveShow.ticket_price_max) : "",
        ticket_url: liveShow.ticket_url || "",
        capacity: liveShow.capacity ? String(liveShow.capacity) : liveShow.available_tickets ? String(liveShow.available_tickets) : "",
        age_restriction: liveShow.age_restriction || "",
      });
    }
  }, [liveShow]);

  const updateLiveShowMutation = useMutation({
    mutationFn: async (liveShowData: typeof formData) => {
      const { data, error } = await supabase
        .from("live_shows")
        .update({
          ...liveShowData,
          address: liveShowData.location,
          name: liveShowData.title,
          show_date: liveShowData.show_date || null,
          show_time: liveShowData.show_time || null,
          capacity: liveShowData.capacity ? Number(liveShowData.capacity) : null,
          duration_minutes: liveShowData.duration_minutes ? Number(liveShowData.duration_minutes) : null,
          ticket_price_min: liveShowData.ticket_price_min ? Number(liveShowData.ticket_price_min) : null,
          ticket_price_max: liveShowData.ticket_price_max ? Number(liveShowData.ticket_price_max) : null,
        })
        .eq("id", liveShow.id)
        .select()
        .single();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateLiveShowMutation.mutate(formData);
  };
        ...
        <CardContent className="p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button
                type="submit"
                disabled={updateLiveShowMutation.isPending}
                className="flex-1 h-10 sm:h-9"
              >
                {updateLiveShowMutation.isPending ? "Updating..." : "Update Live Show"}
              </Button>
