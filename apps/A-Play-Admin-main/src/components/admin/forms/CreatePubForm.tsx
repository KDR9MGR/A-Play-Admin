...
import { X, Beer, MapPin } from "lucide-react";

interface CreatePubFormProps {
  onClose: () => void;
  onSuccess: () => void;
}

export function CreatePubForm({ onClose, onSuccess }: CreatePubFormProps) {
  ...
  const createPubMutation = useMutation({
    mutationFn: async (pubData: typeof formData) => {
      ...
      const { data, error } = await supabase
        .from("pubs")
        .insert([{ ...pubData, address: pubData.location, price_range: pubData.price_range ? Number(pubData.price_range) : null, created_by: user.id }])
        .select()
        .single();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createPubMutation.mutate(formData);
  };
        ...
        <CardContent className="p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button
                type="submit"
                disabled={createPubMutation.isPending}
                className="flex-1 h-10 sm:h-9"
              >
                {createPubMutation.isPending ? "Creating..." : "Create Pub"}
              </Button>
