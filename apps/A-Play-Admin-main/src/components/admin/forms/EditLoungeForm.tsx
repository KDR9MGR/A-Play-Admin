...
interface EditLoungeFormProps {
  lounge: any;
  onClose: () => void;
  onSuccess: () => void;
}

export function EditLoungeForm({ lounge, onClose, onSuccess }: EditLoungeFormProps) {
  ...
  useEffect(() => {
    if (lounge) {
      setFormData({
        ...
        location: lounge.location || lounge.address || "",
        cover_image: lounge.cover_image || "",
        logo_url: lounge.logo_url || "",
        phone: lounge.phone || "",
        email: lounge.email || "",
        website: lounge.website || "",
        price_range: lounge.price_range ? String(lounge.price_range) : "",
        amenities: lounge.amenities || [],
      });
    }
  }, [lounge]);

  const updateLoungeMutation = useMutation({
    mutationFn: async (loungeData: typeof formData) => {
      const { data, error } = await supabase
        .from("lounges")
        .update({ ...loungeData, address: loungeData.location, price_range: loungeData.price_range ? Number(loungeData.price_range) : null })
        .eq("id", lounge.id)
        .select()
        .single();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateLoungeMutation.mutate(formData);
  };
        ...
        <CardContent className="p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button
                type="submit"
                disabled={updateLoungeMutation.isPending}
                className="flex-1 h-10 sm:h-9"
              >
                {updateLoungeMutation.isPending ? "Updating..." : "Update Lounge"}
              </Button>
