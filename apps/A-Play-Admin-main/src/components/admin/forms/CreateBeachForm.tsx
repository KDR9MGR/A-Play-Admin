...
import { X, Waves, MapPin } from "lucide-react";

interface CreateBeachFormProps {
  onClose: () => void;
  onSuccess: () => void;
}

export function CreateBeachForm({ onClose, onSuccess }: CreateBeachFormProps) {
  ...
  const createBeachMutation = useMutation({
    mutationFn: async (beachData: typeof formData) => {
      ...
      const { data, error } = await supabase
        .from("beaches")
        .insert([{ ...beachData, address: beachData.location, price_range: beachData.price_range ? Number(beachData.price_range) : null, created_by: user.id }])
        .select()
        .single();
        ...
        <CardContent className="p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button
                type="submit"
                disabled={createBeachMutation.isPending}
                className="flex-1 h-10 sm:h-9"
              >
                {createBeachMutation.isPending ? "Creating..." : "Create Beach"}
              </Button>
