...
export function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch recent bookings — profiles joined separately (no FK declared on bookings.user_id)
      const { data: rawBookings } = await supabase
        .from('bookings')
        .select('id, amount, status, booking_date, created_at, event_id, user_id, events(title)')
        .order('created_at', { ascending: false })
        .limit(5);

      let recentBookingsData: any[] = rawBookings || [];
      if (rawBookings && rawBookings.length > 0) {
        ...
        const { data: profilesData } = await supabase
          ...
          .select('id, full_name')
          .in('id', userIds);
        recentBookingsData = rawBookings.map(b => ({
          ...b,
          profiles: profilesData?.find(p => p.id === b.user_id) ?? null,
        }));
      }
      ...
      // Only count confirmed bookings for revenue (pending/cancelled should not be counted)
      const totalRevenue = bookingStats
        ?.filter(b => b.status === 'confirmed')
        .reduce((sum, b) => sum + (parseFloat(b.amount?.toString() || '0') || 0), 0) || 0;
